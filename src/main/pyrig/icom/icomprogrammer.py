...
class IcomUtilities:
  ...
  def __init__(self):
    ...
    self.radio_configurations = {
      ...
      'IC-7000-Boat' : ('IC-7000',('general_hf','marine_hf','marine_vhf_wx','general_uhf','emergency_beacon','marine_vhf_long','family_radio_service','cb','E')),
      'IC-7000-Home' : ('IC-7000',('general_hf','marine_hf','marine_vhf_wx','general_uhf','emergency_beacon','marine_vhf_long','family_radio_service','cb','vhf_repeaters_long','E')),
      ...
      'IC-R8500' : ('IC-R8500',('general_hf','marine_hf','marine_vhf_wx','marine_vhf_long','general_uhf','emergency_beacon','family_radio_service','cb','public_service','vhf_repeaters_long','E'))
    }
  ...
  def encode_write_pdf_file(self,radio_name,html_data,pdf_path):
    try:
      ...
      stdoutdata, stderrdata = p.communicate(html_data)
      self.write_file(pdf_path,stdoutdata)
...
class OdsToArray:
  ...
  def extract_simple(self,data,tag):
    return re.findall('(?s)<%s[^/|>]*?>(.*?)</%s>' % (tag,tag),data)
...
class IcomIO:
  ...
  def set_split(self,split, force = False):
  ...
  def program_radio(self,array,radio_id,banksize,radio_tag,erase_unused = False):
    ...
    for mm,record in enumerate(array[1:]):
      ...
      if(rxf and mode): # minimum required
        ...
        if(has_split):
          if(txf): # transmit frequency specified
            self.set_split(True)
            self.set_vfo(1)
  ...
  def process(self):
    ...
    while True:
      ...
      for n,item in enumerate(menu):
        print('    %d: %s' % (n+1,item))
      lm = len(menu)
      sys.stdout.write('Select (1 - %d): ' % lm)
